...
*(NOTE: Because of anti-competitive Google policies (Google reviews), the Chrome Web Store version will only update every major version (e.g. 3.1.0 -> 3.2.0, skipping 3.1.1 - 3.1.14). If you want the latest release, download the .zip file and load it unpacked straight onto your browser, or switch to firefox. Note that the github version will not have auto-update functionality, as Chrome and Chromium blocks all auto-updates not from the Chrome Web Store. Blame Google for why you can't have nice things.)*
